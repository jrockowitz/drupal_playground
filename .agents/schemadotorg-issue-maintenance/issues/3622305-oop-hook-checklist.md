...
| `schemadotorg_jsonapi` | `schemadotorg_mapping_insert` | Converted | `SchemaDotOrgJsonApiHooks::schemadotorgMappingInsert()` | `SchemaDotOrgJsonApiManagerKernelTest` covers resource creation. | Hook-specific lifecycle logic moved into the hook class. |
| `schemadotorg_jsonapi` | `schemadotorg_mapping_update` | Converted | `SchemaDotOrgJsonApiHooks::schemadotorgMappingUpdate()` | `SchemaDotOrgJsonApiManagerKernelTest` covers resource updates. | Hook-specific lifecycle logic moved into the hook class. |
| `schemadotorg_jsonapi` | `field_config_insert` | Converted | `SchemaDotOrgJsonApiHooks::fieldConfigInsert()` | `SchemaDotOrgJsonApiManagerKernelTest` covers field resource creation. | Hook-specific lifecycle logic moved into the hook class. |
...
| `schemadotorg_jsonld_breadcrumb` | `schemadotorg_jsonld` | Converted | `SchemaDotOrgJsonLdBreadcrumbJsonLdHooks::schemadotorgJsonld()` | `SchemaDotOrgJsonLdBreadcrumbKernelTest` covers breadcrumb JSON-LD output. | Hook-only manager service and interface removed; direct test instantiation now uses the hook class. |
| `schemadotorg_jsonld_breadcrumb` | `schemadotorg_jsonld_alter` | Converted | `SchemaDotOrgJsonLdBreadcrumbJsonLdHooks::schemadotorgJsonldAlter()` | `SchemaDotOrgJsonLdBreadcrumbKernelTest` covers JSON-LD placement. | Hook-only manager service and interface removed; no ordering constraint. |
| `schemadotorg_jsonld_embed` | `schemadotorg_jsonld` | Converted | `SchemaDotOrgJsonLdEmbedJsonLdHooks::schemadotorgJsonld()` | `SchemaDotOrgJsonLdEmbedKernelTest` covers embedded entity JSON-LD. | Hook-only manager service and interface removed. |
| `schemadotorg_mapping_set` | `help` | Converted | `SchemaDotOrgMappingSetHelpHooks::help()` | No direct help-output assertion; coverage gap. | Uses `StringTranslationTrait`; no service boundary changed. |
| `schemadotorg_mapping_set` | `form_schemadotorg_mapping_add_form_alter` | Converted | `SchemaDotOrgMappingSetFormHooks::formSchemadotorgMappingAddFormAlter()` | Existing mapping-set functional tests pass; no direct warning assertion. | Injects the retained mapping-set manager, request stack, and messenger; manager is used by controllers, Drush, and tests. |
| `schemadotorg_office_hours` | `schemadotorg_property_field_alter` | Converted | `SchemaDotOrgOfficeHoursHooks::schemadotorgPropertyFieldAlter()` | `SchemaDotOrgOfficeHoursJsonLdKernelTest` covers the configured Office Hours field. | Hook-only manager service and interface removed. |
| `schemadotorg_office_hours` | `schemadotorg_jsonld_schema_property_alter` | Converted | `SchemaDotOrgOfficeHoursJsonLdHooks::schemadotorgJsonldSchemaPropertyAlter()` | `SchemaDotOrgOfficeHoursJsonLdKernelTest::testOpening()` covers OpeningHoursSpecification JSON-LD. | Hook-only JSON-LD manager service and interface removed; test `@covers` reference updated. |
| `schemadotorg_physical` | `schemadotorg_jsonld_schema_property_alter` | Converted | `SchemaDotOrgPhysicalJsonLdHooks::schemadotorgJsonldSchemaPropertyAlter()` | `SchemaDotOrgPhysicalJsonLdKernelTest::testJsonLdPhysical()` covers QuantitativeValue JSON-LD. | Hook-only JSON-LD manager service and interface removed; test `@covers` reference updated. |
| `schemadotorg_report` | `help`, `menu_local_tasks_alter`, `form_schemadotorg_general_settings_form_alter` | Converted | `SchemaDotOrgReportHelpHooks::help()`, `SchemaDotOrgReportModuleHooks::menuLocalTasksAlter()`, `SchemaDotOrgReportFormHooks::formSchemadotorgGeneralSettingsFormAlter()` | Report functional tests cover settings and report pages; no direct help/local-task assertion. | Injects schema type manager; update hook retained procedurally. |
| `schemadotorg_simple_sitemap` | `schemadotorg_mapping_insert`, `form_schemadotorg_mapping_set_confirm_form_alter` | Converted | `SchemaDotOrgSimpleSitemapHooks::mappingInsert()`, `SchemaDotOrgSimpleSitemapFormHooks::formSchemadotorgMappingSetConfirmFormAlter()` | `SchemaDotOrgSimpleSitemapKernelTest` passes. | Hook-only manager service/interface removed; submit handler is a class method. |
| `schemadotorg_smart_date` | `schemadotorg_property_field_alter`, `schemadotorg_jsonld_schema_type_field_alter` | Converted | `SchemaDotOrgSmartDateHooks::schemadotorgPropertyFieldAlter()`, `SchemaDotOrgSmartDateJsonLdHooks::schemadotorgJsonldSchemaTypeFieldAlter()` | Smart Date kernel tests pass. | Both managers retained: JSON-LD manager has focused existing tests and reusable behavior. |
| `schemadotorg_content_model_documentation` | all ten runtime hooks | Converted | `SchemaDotOrgContentModelDocumentationHooks`, `...HelpHooks`, and `...FormHooks` methods | Content-model functional and kernel tests pass. | Manager is used by builder; builder provides reusable behavior. Install/update functions and settings submit callback remain procedural. |
| `schemadotorg_diagram` | `help`, `local_tasks_alter`, `form_schemadotorg_general_settings_form_alter` | Converted | `SchemaDotOrgDiagramHelpHooks::help()`, `...ModuleHooks::localTasksAlter()`, `...FormHooks::formSchemadotorgGeneralSettingsFormAlter()` | Diagram functional tests pass. | Update functions remain procedural; settings submit handler is a form-hook class method. |
...
| `schemadotorg_jsonld_breadcrumb` | `schemadotorg_jsonld_breadcrumb_install()`, `schemadotorg_jsonld_breadcrumb_update_10000()`, `schemadotorg_jsonld_breadcrumb_update_10001()` | Drupal install/update entry points must remain procedural. | No OOP equivalent. | Focused JSON-LD test passed; install/update execution remains outside this conversion. |
| `schemadotorg_office_hours` | `schemadotorg_office_hours_install()`, `schemadotorg_office_hours_uninstall()` | Drupal install/uninstall entry points must remain procedural. | `SchemaDotOrgOfficeHoursInstallTest` covers installation. | No OOP equivalent. |
| `schemadotorg_physical` | `schemadotorg_physical_install()`, `schemadotorg_physical_uninstall()` | Drupal install/uninstall entry points must remain procedural. | `SchemaDotOrgPhysicalKernelTest` covers installation-dependent behavior. | No OOP equivalent. |

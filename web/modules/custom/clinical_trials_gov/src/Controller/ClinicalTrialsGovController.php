...
class ClinicalTrialsGovController extends ControllerBase {
  ...
  public function index(): array {
    ...
    if (!$query) {
      $message = $this->buildMessages($this->t('Please go to <a href=":url">Find</a> and build your query.', [
        ':url' => Url::fromRoute('clinical_trials_gov.find')->toString(),
      ]));
    }
    elseif (!$paths) {
      $message = $this->buildMessages($this->t('Your query is saved, but no fields were discovered. Go back to <a href=":url">Find</a> and save a query that returns studies.', [
        ':url' => Url::fromRoute('clinical_trials_gov.find')->toString(),
      ]));
    }
    elseif (!$type || !$field_mappings) {
      $message = $this->buildMessages($this->t('Your query is saved. Go to <a href=":url">Configure</a> and select the destination content type and fields.', [
        ':url' => Url::fromRoute('clinical_trials_gov.configure')->toString(),
      ]));
    }
    else {
      $message = $this->buildMessages($this->t('Your query and field mapping are ready. Continue to <a href=":url">Import</a> when you are ready to sync studies.', [
